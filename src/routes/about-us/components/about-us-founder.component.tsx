    ...
    <main className="px-5 md:px-20 md:mt-24 mt-16 flex flex-col md:flex-row-reverse">
      <section className="bg-nature-600 rounded-t-lg md:rounded-tl-none md:rounded-r-2xl p-4 md:p-8">
        <div className="flex flex-col md:flex-row md:justify-between md:w-full">
          <div>
            <p className="text-lg md:text-[28px] font-black font-nunito">
              {lang === 'fa' ? 'دکتر پژمان صدقانی' : 'Dr. Pejman Sedghani'}
            </p>
            <p className="mt-2 md:text-2xl">
              {lang === 'fa' ? 'بنیان‌گذار ' : 'Founder of '}
        ...
        <p className="text-sm mt-4 md:mt-6 md:text-base leading-6 md:leading-7">
          {lang === 'fa'
            ? 'در سال 1999، دکتر پژمان صدقانی، در حالی که دانشجوی دکتری دامپزشکی و کارآموز در یک کلینیک دامپزشکی بود، نیاز به غذا و محصولات مرتبط برای حیوانات خانگی را متوجه شد و تصمیم به واردات محصولات حیوانات خانگی با کمک پدرش که صاحب یک شرکت بازرگانی بود، گرفت. او اولین واردکننده این محصولات شد و این صنعت را در کشور گسترش داد. در طول سال‌ها، با کسب ده‌ها نمایندگی از سرتاسر جهان، از جمله چین، هلند، آلمان و غیره، توانست سهم بزرگی از بازار کشور را به دست آورد.'
            : 'In 1999, Dr. Pejman Sedghani, while he was a veterinary doctorate student and an intern at a veterinary clinic, noticed the need for pets in food and related products, and decided to import pet products with the help of his father, who owned a trading company. He became the first importer of these products and expanded this industry in this country. Over the years, by obtaining dozens of representatives from all over the world, including China, Holland, Germany, etc., he was able to have a large share of the market in the country.'}
        </p>
      </section>
      <img
