...
const OrderCompleteDetails = ({ lang = 'en' }) => {
  ...
  return (
    <section className="flex flex-col items-center mt-10 md:mt-16 px-5">
      ...
      <h1 className="mt-3 md:mt-4 font-bold md:text-2xl text-center">
        {lang == 'fa' ? 'سفارش شما انجام شد' : 'Your order is completed'}
      </h1>
      <h1 className="mt-2 md:mt-4 text-sm md:text-lg text-center px-3">
        {lang == 'fa'
          ? 'کارشناسان ما پس از بررسی سفارش با شما تماس خواهند گرفت.'
      ...
      <div className="flex flex-col md:flex-row gap-4 w-full mt-6 md:mt-8 md:w-[593px]">
        <Button
          ...
          onClick={() => {
            lang == 'fa'
              ? router.push(`/fa/dashboard/orders/${orderId}`)
        ...
        >
          {isCopied ? (
            <>
              <SvgCopyPaste />
              {lang == 'fa' ? 'کپی شده' : 'Copied'}: #{orderId}
            </>
          ) : (
            <>
              <SvgCopyPaste />
              {lang == 'fa' ? 'شناسه سفارش' : 'Order ID'}: #{orderId}
            </>
          )}
        </Button>
      </div>
